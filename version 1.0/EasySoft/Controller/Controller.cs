...
namespace EasySoft.controller
{
    class Controller
    {
        ...
        private void OpenBackup()
        {
            ...
            view.ShowNameFile();
            var jsonString = File.Exists(model.BackupListFile)
                ? File.ReadAllText(model.BackupListFile)
                : "";
            var list = !string.IsNullOrWhiteSpace(jsonString)
                ? JsonConvert.DeserializeObject<Backup[]>(jsonString)!
                : Array.Empty<Backup>();
